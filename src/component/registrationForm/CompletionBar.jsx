...
const CompletionBar = ({ currentStep }) => {
  ...
  return (
    <div className="flex justify-between items-center w-full max-w-4xl mx-auto my-8">
      {steps.map((step, index) => (
        <div key={index} className="flex-1 flex flex-col">
          <div className="flex items-center w-full">
            <div className={`flex items-center justify-center h-10 w-10 rounded-full ${step.completed ? 'bg-blue-400 text-white' : 'bg-gray-100 text-gray-100'}`}>
              {step.logo}
            </div>
            {index < steps.length - 1 && (
              <div className={`flex-1 h-0.5 ${steps[index + 1].completed ? 'bg-blue-400' : 'bg-blue-100'}`}></div>
            )}
          </div>
          <span className={`mt-2 ${step.completed ? 'text-black' : 'text-gray-500'}`}>
            {step.label}
          </span>
        </div>
      ))}
